...
SEED_GRPC_PORT="${SEED_GRPC_PORT:-$((GRPC_PORT_BASE + NODE_ID_OFFSET + 1))}"
...
for i in $(seq 1 "$NODE_COUNT"); do
  ...
  port=$((GRPC_PORT_BASE + node_id))
  printf "node-%-3s %s\n" "$node_id" "$(grpcurl -plaintext -d '{}' -proto "$PROTO_FILE" "127.0.0.1:${port}" orionkv.node.ClusterRpc/GetMembership >/dev/null 2>&1 && echo UP || echo DOWN)"
done
