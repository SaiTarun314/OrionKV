...
CLIENT_RESTART_URL="${CLIENT_RESTART_URL:-}"
CLIENT_RESTART_METHOD="${CLIENT_RESTART_METHOD:-POST}"
CLIENT_RESTART_BODY="${CLIENT_RESTART_BODY:-}"
CLIENT_RESTART_REQUIRED="${CLIENT_RESTART_REQUIRED:-false}"
CLIENT_RESTART_WAIT_SECONDS="${CLIENT_RESTART_WAIT_SECONDS:-5}"
...
if [[ -z "$CLIENT_REFRESH_SEED_GRPC_ADDRESS" ]]; then
  CLIENT_REFRESH_SEED_GRPC_ADDRESS="${SEED_HOST_IP}:$((19090 + SEED_NODE_ID))"
fi
...
Optional client reset:
  CLIENT_RESTART_URL=http://client-host:port/path
  CLIENT_RESTART_METHOD=POST
  CLIENT_RESTART_BODY='{"reset":true}'
...
Examples:
  ...
  CLIENT_RESTART_URL=http://152.7.177.154:8090/client/restart \
